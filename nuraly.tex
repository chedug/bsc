...
The objective of simple linear regression is, under some assumption \cite[p.4-12]{hayashi2011econometrics}, to estimate the parameters $\beta_0$ and $\beta_1$, so that they will provide best fit. The assumptions are important since they serve as a foundation for later theory, so we will take some to analyze them:
...
\begin{proof}
	\begin{align*}
		\E (x_i \epsilon_j) &= \E (\E(x_i \epsilon_j| x_j))\tag*{(by law of total expectation)} \\
		& =\E (x_i\E( \epsilon_j| x_j)) \tag*{(by linearity of expectation)} \\
		&= \E (x_i 0) 	= 0
	\end{align*}
\end{proof}
...
\textbf{Remark:} \todo{ask professor} Although in the assumptions we treat regressors as random variables, in reality, $x_1, ..., x_n$ are realizations of random variables, so it is generally accepted to tream them as fixed 
...
\begin{proposition} \label{prop:normally}
	The difference of the two estimators is normally distributed as follows: \todo{rm}
	\[
...
\begin{proposition}
	$\E [\hat{\beta_1^0}-\hat{\beta_1}]=\beta_1^0-\beta_1$ \todo{not true in general}
\end{proposition}
...
\begin{proof}[Proof of proposition \ref{prop:ssd_full_model}]
	\begin{align*}
		...
		(\ast) &= n \beta_0^2+2\beta_0 \beta_1 n \bar{x} + \beta_1^2 S_{xx}^o - \sum 2(\beta_1x+\beta_0)^2+\frac{\sigma^2}{}
	\end{align*}
\end{proof}
